...
"""
# KNN预测模型
def my_cart(featers,labels,class_list,k,is_abs=True):
    ...
    mean_accuracy_rate = sum_accuracy_rate/5
    mean_time = sum_time/5

    # K近邻模型的性能与样本判别执行平均时间
    return mean_accuracy_rate,mean_time
...
def main():
    ...
    result = pd.DataFrame(accuracy_rate_list, index=k_list, columns=['绝对值距离', '绝对值距离_时间', '欧式距离', '欧式距离_时间'])
    print(result)
