...
# parameters3_1 = {'n_estimators':range(10,300,5)}
# parameters3_2 = {'max_depth':range(1,20),'min_samples_split':range(2,10)}  #搜索空间太大，分两次调整参数
...
30 total features after one-hot encoding.
{'n_estimators': 10}
searching time for RandomForestClassifier is 75.306 s
{'max_depth': 9, 'min_samples_split': 8}
searching time for RandomForestClassifier is 14.185 s
"""
clf = RandomForestClassifier(random_state=29,n_estimators=10,max_depth=9,min_samples_split=8)
x = features
y = labels
clf.fit(x,y)
...
def save(clf,i):
    ...
    sub.to_csv("随机森林2_{}.csv".format(i), index=False)
