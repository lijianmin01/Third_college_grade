...
# # parameters6_1 = {'n_estimators':range(10,300,5)}
# # parameters6_2 = {'max_depth':range(1,20),'min_child_weight':range(1,20)}
# parameters6_3 = {'subsample':[i/10.0 for i in range(1,10)], 'colsample_bytree':[i/10.0 for i in range(1,10)]}#搜索空间太大，分三次调整参数

"""
{'n_estimators': 55}
searching time for XGBClassifier is 81.523 s
{'max_depth': 4, 'min_child_weight': 8}
searching time for XGBClassifier is 287.144 s
{'colsample_bytree': 0.2, 'subsample': 0.4}
searching time for XGBClassifier is 72.793 s
"""

# clf=fit_model(alg6,parameters6_1)
# clf=fit_model(alg6,parameters6_2)
#clf=fit_model(alg6,parameters6_3)

clf = XGBClassifier(random_state=29,n_jobs=-1,n_estimators=55,max_depth=6,min_child_weight=8,colsample_bytree=0.2,subsample=0.4)
x = features
y = labels
clf.fit(x,y)
...
def save(clf,i):
    ...
    sub.to_csv("XGBClassifier1_{}.csv".format(i), index=False)
